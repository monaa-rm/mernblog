...
export async function GET(req, { params: { id } }) {
  try {
    ...
    const on_ads = searchParams.get("on") == 1 ? 1 : 0;
    if (on_ads == 0) {
      const advertises = await Advertise.find({ user_id: id })
        .sort({ _id: -1 })
        .skip((page_number - 1) * paginate)
        .limit(paginate);

      const all_advertises_number = (await Advertise.find({ user_id: id }))
        .length;
      ...
      const all_advertises_number = (await Advertise.find({ published: true }))
        .length;
      return NextResponse.json(
        { all_advertises_number, advertises },
        { status: 200 }
