...
const MyDraftPost = ({pagination_url}) => {
  ...
  useEffect(() => {
    const url = `/api/post/all-posts?paginate=${paginate}&page_number=${page_number}&draft=1`;
    axios
  ...
  return (
    <div className="flex flex-col gap-8 w-full">
            <>
        <title>پیش نویس های من</title>
      </>
      <div className="flex items-center justify-between gap-2 ">
        <div className="text-xl">پیش نویس های من</div>
      </div>
      ...
      <div className="flex justify-center items-center gap-4">
        { all_posts.length == 0 ? <div></div>  : btns != -1 &&
          btns.map((btn, i) => (
            <Link
              onClick={() => {
                setall_posts(-1);
