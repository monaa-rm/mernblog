...
const PostsNotAccepted = ({ pagination_url }) => {
  ...
  useEffect(() => {
    const url = `/api/dashboard/post/sended-to-accept?paginate=${paginate}&page_number=${page_number}&not_accepted_posts=1`;
    axios
  ...
  return (
    <div className="flex flex-col gap-8 w-full">
      <div className="flex items-center justify-between gap-2 ">
        <div className="text-xl pt-12 md:pt-0"> پست های در انتظار انتشار</div>
        <Link
          className="px-3 py-2 z-10 fixed top-[8.7rem] sm:top-[8.3rem] text-white rounded-md left-12 bg-blue-500 hover:bg-blue-600 transition-all duration-500 "
        ...
        {allPosts == -1 ? (
          ...
          <div className="flex flex-col gap-8 w-full">
            <div className="flex flex-col gap-4 w-full">
              {allPosts.map((item, i) => (
                <Link
                  href={`/dashboard/not-accepted/${item._id}`}
                  key={i}
...
export default PostsNotAccepted;
