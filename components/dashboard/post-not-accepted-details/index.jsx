...
const PostsNotAcceptedDetails = ({ goalId }) => {
  ...
  const formSubmiter = (e) => {
    ...
    axios
      ...
      .then((data) => {
        ...
        router.push("/dashboard/not-accepted");
      })
      .catch((error) => {
        console.log(error);
  ...
  const postAccepter = () => {
    ...
    axios
      .post(`/api/dashboard/post/accept-post`, formData, {
        headers: { token: token },
      })
      .then((data) => {
        toast.success("پست با موفقیت تایید شد", {
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
          pauseOnHover: true,
  ...
  const postNotAccepter = (e) => {
    e.preventDefault();
    const formData = {
      post_id: goalId,
      user_id: PostData.author_id,
      text: notifRef.current.value,
    };
    axios
      .post(`/api/dashboard/post/accept-post-not`, formData, {
        headers: { token: token },
      })
      .then((data) => {
        toast.success(" عدم تایید پست با موفقیت انجام شد", {
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
          pauseOnHover: true,
  ...
  return (
    <div className="flex flex-col gap-2 w-full ">
      <>
        <title>جزئیات پست در انتظار تایید</title>
      </>
      ...
      <div className="w-full flex flex-col gap-2">
        {PostData == -1 || categories == -1 ? (
          ...
          <div className="flex flex-col gap-4 my-8">
            ...
            <div className=" mx-4 my-8 p-4 rounded bg-zinc-200 dark:bg-zinc-600  flex flex-col gap-12 ">
              <div className="flex justify-start gap-4 ">
                <div className="w-full h-8 hidden md:flex"></div>
                <button
                  onClick={postAccepter}
                  className="bg-green-600 rounded-md flex justify-center items-center w-36 h-8 transition-all duration-500 hover:bg-green-700 text-white "
                >
                  پست تایید شد
                </button>
              </div>
              <form className="flex justify-start gap-4 items-center flex-wrap">
                <input
                  required
                  ref={notifRef}
                  type="text"
                ...
                <button
                  type="button"
                  onClick={postNotAccepter}
                  className="bg-rose-600 rounded-md flex justify-center items-center w-36 min-w-36 h-8 transition-all duration-500 hover:bg-rose-700 text-white "
                >
                  پست تایید نشد
                </button>
              </form>
...
export default PostsNotAcceptedDetails;
