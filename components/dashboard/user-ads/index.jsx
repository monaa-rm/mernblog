...
const UserAds = ({ pagination_url,goalId }) => {
...
const [removeUrl, setRemoveUrl] = useState("");
  ...
  useEffect(() => {
    const url = `/api/dashboard/user/user-ads/${goalId}?paginate=${paginate}&page_number=${page_number}`;
    axios
  ...
  return (
    <div className="flex flex-col gap-8 w-full">
            <>
        <title>همه تبلیغ های این کاربر</title>
      </>
      <div className="flex items-center relative justify-between gap-2 ">
        <div className="text-xl pt-12 md:pt-0">همه تبلیغ های این کاربر</div>
        <Link
          className="px-3 py-2 z-10 static lg:fixed top-[8.7rem] md:top-[8.3rem] text-white rounded-md left-12 md:left-24 lg:left-12 bg-blue-500 hover:bg-blue-600 transition-all duration-500 "
          href={"/dashboard/on-ads"}
        >
         تبلیغ های روشن
        </Link>
      </div>
          ...
          <div className="flex flex-col gap-8 w-full">
            <div className="flex flex-col gap-12 w-full">
              {allAdvertises.map((item, i) => (
                <div
                  key={i}
                  className=" w-full relative flex flex-col items-center gap-2 bg-white dark:bg-zinc-800 border-2 border-zinc-200 hover:border-blue-600   rounded-md p-6 transition-all duration-300 hover:bg-zinc-200 "
                >
                  <div className="flex items-center justify-center md:justify-between flex-wrap gap-4">
                    <div className="text-sm bg-yellow-500 text-white rounded-md w-24 py-2 flex justify-center items-center">
                      {item.type == "banner"
                        ? "تبلیغ بنر"
                        : item.type == "post"
                        ? "تبلیغ پست"
                    ...
                    <div className="flex items-center flex-wrap justify-center md:justify-end gap-4">
                      <button
                        ...
                        className="text-sm bg-rose-600 hover:bg-rose-700 text-white rounded-md px-3 py-2 flex justify-center items-center"
                      >
                        حذف
                      </button>
                      {item.viewed == true ? (
                        <button
                          ...
                          className="text-sm bg-green-500 hover:bg-green-600 text-white rounded-md px-3 py-2 flex justify-center items-center"
                        >
                          بررسی شده
                        </button>
                      ) : (
                        <button
                          ...
                          className="text-sm bg-rose-500 hover:bg-rose-600 text-white rounded-md px-3 py-2 flex justify-center items-center"
                        >
                          بررسی نشده
                        </button>
                      )}
                  ...
                  <div className="flex justify-center md:justify-between flex-wrap items-center gap-4">
                    <Link
                      href={item.link}
                      target="_blank"
                      className="px-3 py-2  flex justify-center items-center rounded-lg bg-blue-500 transition-all duration-500 text-white hover:bg-blue-600 "
                    >
                      لینک هدف
                    </Link>
                    <Link
                    ...
                    <div className="text-sm bg-blue-300 text-white rounded-md px-3 py-2 flex justify-center items-center">
                  ...
                  {item.image && item.image != "no-img" ? (
                    <div className="my-2 flex justify-center md:justify-between flex-wrap-reverse items-center md:items-end gap-4">
