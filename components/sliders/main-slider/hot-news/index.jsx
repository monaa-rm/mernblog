...
const HotNews = ({posts_ads}) => {
  ...
  const newsList = posts_ads.map((ads, i) => ({ ...ads, value: i }));

  const [activeNews, setactiveNews] = useState(newsList[0] || "");
  if (posts_ads.length > 0) {
    setTimeout(() => {
  ...
  return (
    <div className=" flex justify-start items-center gap-1">
      {posts_ads.length > 0 ? (
        <>
          <span className="title_style w-24 min-w-[96px] dark:text-white  xl:w-24 xl:min-w-[96px] ">داغ ترین ها : </span>
          <Link
            target="_blank"
            rel="noreferrer"
