...
const SignInComponent = () => {
   ...
   const signIner = () => {
      ...
      .then(data=>{
         ...
         dispatch(setusernameValue(data.data.data.blog_slug));

         setuserBlogSlug(data.data.data.blog_slug);
         router.push(`/blog/${data.data.data.blog_slug}`)
      })
      .catch((error)=>{
         ...
         <form onSubmit={handleSubmit(signIner)} className=' flex flex-col gap-8 p-4'>
            <div>
               <input
                  ...
                  type="text"
                  placeholder='9 1 2 * * * * * * * * '
                  className='ltr_dir border-2 border-zinc-200 dark:text-black rounded-md p-2 outline-none focus:border-blue-500 w-full '
                  {
                  ...register("phone",
                     {
                        required: true,
            ...
            <div>
               <input
                  ...
                  className='border-2 border-zinc-200 dark:text-black rounded-md p-2 outline-none focus:border-blue-500 w-full '
                  {
                  ...register("password",
                     {
                        required: true,
